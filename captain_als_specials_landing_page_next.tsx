...
type SpecialsKey = "lunch" | "happy" | "ten" | "chef";
  ...
  switch (key) {
    case "lunch":
      return {
        ...
        subtle: "border-emerald-200 bg-emerald-50 text-emerald-900",
    ...
    case "happy":
      return {
        ...
        subtle: "border-sky-200 bg-sky-50 text-sky-900",
        button: "bg-sky-700 text-white hover:bg-sky-800 focus:ring-sky-700/25",
        ring: "focus:ring-sky-700/25",
...
export default function CaptainAlsSpecialsLanding() {
  const [active, setActive] = useState<SpecialsKey>("lunch");
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-950">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3">
          <Link href="/" className="flex min-w-0 items-center gap-3 text-white">
            ...
            <div className="min-w-0 leading-tight">
              ...
              <div className="truncate text-xs text-slate-300">Specials + monthly giveaway</div>
            </div>
          </Link>
      ...
      <section className="relative isolate min-h-[calc(100svh-65px)] overflow-hidden">
        <img
          src={activeCategory.image.src}
          alt=""
          aria-hidden="true"
          className="absolute inset-0 h-full w-full object-cover opacity-60 blur-sm scale-105"
        />
        <div className="absolute inset-0 bg-slate-950/70" />

        <div className="relative mx-auto grid min-h-[calc(100svh-65px)] max-w-7xl gap-8 px-4 py-8 lg:grid-cols-[minmax(0,0.95fr)_minmax(320px,0.65fr)] lg:items-center lg:py-12">
          <div className="max-w-3xl text-white">
            <div className="inline-flex rounded-lg border border-white/25 bg-white/10 px-3 py-1 text-xs font-semibold backdrop-blur">
              Monthly $100 gift card drawing
            </div>
            <h1 className="mt-4 max-w-3xl text-4xl font-semibold tracking-tight sm:text-5xl">
              Weekly specials in your pocket.
            </h1>
            <p className="mt-4 max-w-xl text-base leading-7 text-slate-200">
              Get Captain Al&apos;s specials by text and email. Enter once a month for a chance to win a $100 gift card.
            </p>
            ...
            <div className="mt-5 grid max-w-2xl gap-3 sm:grid-cols-3">
              <div className="rounded-lg border border-white/15 bg-white/10 p-4 backdrop-blur">
                <div className="text-xs font-semibold text-slate-300">Fast</div>
                <div className="mt-1 text-sm font-semibold text-white">1-minute signup</div>
              </div>
              <div className="rounded-lg border border-white/15 bg-white/10 p-4 backdrop-blur">
                <div className="text-xs font-semibold text-slate-300">Useful</div>
                <div className="mt-1 text-sm font-semibold text-white">Weekly updates</div>
              </div>
              <div className="rounded-lg border border-white/15 bg-white/10 p-4 backdrop-blur">
                <div className="text-xs font-semibold text-slate-300">Simple</div>
                <div className="mt-1 text-sm font-semibold text-white">$100 monthly drawing</div>
              </div>
            </div>

            <div className="mt-4 text-xs text-slate-300">
              No purchase necessary. One entry per phone number per month. Reply STOP to cancel texts.
            </div>
          </div>
          ...
          <div className="rounded-lg border border-white/15 bg-white p-3 shadow-2xl shadow-slate-950/30">
            <div className="flex items-start justify-between gap-4 px-1 pb-3">
              <div>
                <div className="text-sm font-semibold text-slate-950">Current preview</div>
                <div className="mt-1 text-sm text-slate-600">{activeCategory.label}</div>
              </div>
            ...
            <div className="overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
              <img
                ...
                className="h-auto max-h-[54svh] w-full object-contain"
              />
            </div>
          </div>
        </div>
      ...
      <section id="specials" className="bg-slate-50 px-4 py-12">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              <h2 className="text-2xl font-semibold tracking-tight text-slate-950">View current specials</h2>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
                Pick a category. The current graphic updates instantly, and you can swipe the viewer on mobile.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <a
                href="tel:228-831-5751"
                className="hidden rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-950 shadow-sm hover:bg-slate-100 sm:inline-flex"
              >
                Call to order
              </a>
              <button
          ...
          <div
            className={["mt-6 overflow-hidden rounded-lg border bg-white shadow-sm", accent.border].join(" ")}
          ...
          <footer className="mt-10 border-t border-slate-200 pb-24 pt-6 text-xs text-slate-500 sm:pb-0">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>© {new Date().getFullYear()} Captain Al&apos;s</div>
      ...
      {formOpen ? (
        ...
        >
          ...
          >
            <div className={["-mx-5 -mt-5 mb-4 h-2 rounded-t-lg sm:-mx-6 sm:-mt-6", accent.button.split(" ")[0]].join(" ")} />
